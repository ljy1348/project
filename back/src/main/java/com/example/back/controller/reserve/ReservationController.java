...
//            서버 에러
    ...
    // 예약 삭제
    @Transactional
    @DeleteMapping("/deleteRoundTrip/{start}/{end}")
    public ResponseEntity<?> deleteByRound(@PathVariable int start, @PathVariable int end) {
        try {

        reservationService.delete(start);
        reservationService.delete(end);
